...
Eigen::Vector3f compute_triplet_feature(Eigen::Vector3f diff,
  ...
  Eigen::Vector3f v = normal1.cross(diff / (diff.norm() + 1e-6));
...
void FPFHEstimator::compute(std::vector<FPFHSignature33> &fpfh_descriptor) {
  ...
  FPFHResultset fpfh_result_set; // add_histogram(std::vector<float> histogram,
  ...
  for (int i = 0; i < num_keys; i++) {
    ...
    for (int idx : rnn_idx) {
      ...
      float weight = 1.f / (diff.norm() + 1e-8) / num_rnn_point;
