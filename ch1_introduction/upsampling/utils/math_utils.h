...
template <> cv::Vec2f mean(const std::vector<cv::Vec2f> &vec) {
  assert(!vec.empty());
...
generate_gauss_data(int num, const std::array<T, Dim> &means,
...
std::vector<double> generate_gmm_data(int num_want,
...
double compute_gaussian_pdf(double mean, double var, double sample) {
