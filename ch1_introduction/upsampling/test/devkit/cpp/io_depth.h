...
public:
  ...
  DepthImage(const std::string file_name) { readDepthMap(file_name); }
  ...
  DepthImage(const float *data, const int32_t width, const int32_t height)
  ...
  DepthImage(const int32_t width, const int32_t height)
      : width_(width), height_(height) {
    data_ = (float *)malloc(width * height * sizeof(float));
  ...
  void write(const std::string file_name) { writeDepthMap(file_name); }
  ...
  png::image<png::rgb_pixel> errorImage(DepthImage &D_gt,
      ...
      for (int32_t u = 1; u < width() - 1; u++) {
        if (D_gt.isValid(u, v)) {
          ...
          if (log_colors) {
            ...
            float d_err =
                std::min(fabs(getDepth(u, v) - D_gt.getDepth(u, v)), 5.0) / 5.0;
  ...
  void writeDepthMap(const std::string file_name) {
    ...
    for (int32_t v = 0; v < height_; v++) {
      for (int32_t u = 0; u < width_; u++) {
        if (isValid(u, v))
          image.set_pixel(u, v,
                          (uint16_t)(std::max(getDepth(u, v) * 256.0, 1.0)));
        else
  ...
  void writeFalseColors(const std::string file_name, float max_val) {
    ...
    for (int32_t v = 0; v < height_; v++) {
      for (int32_t u = 0; u < width_; u++) {
        ...
        uint8_t r =
            (uint8_t)((w * map[i][0] + (1.0 - w) * map[i + 1][0]) * 255.0);
        uint8_t g =
            (uint8_t)((w * map[i][1] + (1.0 - w) * map[i + 1][1]) * 255.0);
        uint8_t b =
            (uint8_t)((w * map[i][2] + (1.0 - w) * map[i + 1][2]) * 255.0);
