...
struct BGR {
  ...
  const uchar operator[](int idx) const { return this->bgr_(idx); }
  ...
  static bool is_in_radius(const BGR *center, const BGR *data, int axis,
                           int radius) {
...
int main(int argc, char **argv) {
  ...
  std::cout << "Finish building KDTree, which costs \n"
            << tictoc::toc() / 1e3 << "milliseconds\n";
  ...
  std::cout << "@@@@@@@@@RNN Search costs " << tictoc::toc() / 1e3
            << "miliseconds" << '\n';
  ...
  std::cout << "@@@@@@@@@Brute force costs " << tictoc::toc() / 1e3
            << "miliseconds" << '\n';
